...
function createDrawInterval(){
    fetch('http://13.37.56.107:3000/drawScreen').then(x => x.json()).then(r => drawInit(r));
    setInterval(drawScreen, 60);
}

// Recibe un array con los rectángulos y su información para pintar en la pantalla
function drawScreen(){
    fetch('http://13.37.56.107:3000/drawScreen').then(x => x.json()).then(arr => askDrawScreen(arr));
    fetch('http://13.37.56.107:3000/fruits').then(x => x.json()).then(f => printFruits(f));
}
